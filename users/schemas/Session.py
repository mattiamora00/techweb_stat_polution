...
class Query(graphene.ObjectType):
    ...
    def resolve_session_user_id(self,info,token):
        ...
        if len(session) == 1:
            ...
            return json.dumps({"userId": user.id, "userData": {"id":user.id,"username":user.username,"email":user.email ,"imageProfile":user.profile_image.name,"viewPlan":user.view_plan,"viewSensor":user.view_sensor,"viewSick":user.view_sick,"viewGraph":user.view_graph,"password":user.password, "name":user.name,"surname":user.surname}})
        else:
            return -1
